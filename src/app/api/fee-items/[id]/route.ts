...
export async function GET(
  ...
  try {
    ...
    const feeItem = await db.collection('fee_items').findOne({ _id: new ObjectId(params.id) })
    
    if (feeItem) {
      return NextResponse.json(serializeDoc(feeItem))
    } else {
...
export async function PUT(
  ...
  try {
    ...
    const result = await db.collection('fee_items').updateOne(
...
export async function DELETE(
  ...
  try {
    ...
    const result = await db.collection('fee_items').deleteOne({ _id: new ObjectId(params.id) })
